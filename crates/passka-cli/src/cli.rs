...
pub enum PrincipalCommand {
    ...
    Token {
        #[command(subcommand)]
        command: PrincipalTokenCommand,
    },
}

#[derive(Subcommand)]
pub enum PrincipalTokenCommand {
    Issue { principal_id: String },
    Revoke { principal_id: String },
}
...
pub enum AccountCommand {
    ...
    Allow {
        ...
        #[arg(long = "allow-host", value_delimiter = ',')]
        allowed_hosts: Vec<String>,
        #[arg(long = "allow-method", value_delimiter = ',')]
        allowed_methods: Vec<String>,
        #[arg(long = "allow-path-prefix", value_delimiter = ',')]
...
mod tests {
    ...
    fn account_allow_is_the_primary_account_auth_flow() {
        ...
        match cli.command {
            Command::Account {
                ...
                assert!(allowed_path_prefixes.is_empty());
    ...
    #[test]
    fn account_allow_parses_scope_flags() {
        let cli = Cli::try_parse_from([
            "passka",
            "account",
            "allow",
        ...
        .expect("account allow scope flags should parse");
        ...
        match cli.command {
            Command::Account {
                ...
                assert_eq!(allowed_hosts, vec!["api.openai.com", "localhost:3000"]);
                assert_eq!(allowed_methods, vec!["GET", "POST"]);
                assert_eq!(allowed_path_prefixes, vec!["/v1/models", "/v1/chat"]);
            }
            _ => panic!("unexpected command shape"),
        }
    }
    ...
    #[test]
    fn principal_token_issue_is_available() {
        let cli = Cli::try_parse_from([
            "passka",
            "principal",
            "token",
        ...
        .expect("principal token issue should parse");

        match cli.command {
            Command::Principal {
                command:
                    PrincipalCommand::Token {
                        command: PrincipalTokenCommand::Issue { principal_id },
                    },
            } => assert_eq!(principal_id, "principal:local-agent"),
            _ => panic!("unexpected command shape"),
        }
    }
}
