...
pub fn run(account_id: &str) -> Result<()> {
    ...
    let redirect_uri = redirect_uri_from_authorization_url(&session.authorization_url)
...
fn redirect_uri_from_authorization_url(auth_url: &str) -> Result<String> {
    let url = url::Url::parse(auth_url).context("invalid authorization URL")?;
    url.query_pairs()
