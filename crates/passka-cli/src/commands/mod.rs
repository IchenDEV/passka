...
use crate::cli::{
    AccountCommand, AuditCommand, BrokerCommand, Command, PrincipalCommand, PrincipalTokenCommand,
};
use anyhow::Result;
    ...
    match cmd {
        Command::Principal { command } => match command {
            ...
            PrincipalCommand::Token { command } => match command {
                PrincipalTokenCommand::Issue { principal_id } => principal::issue_token(&principal_id),
                PrincipalTokenCommand::Revoke { principal_id } => principal::revoke_token(&principal_id),
            },
        },
